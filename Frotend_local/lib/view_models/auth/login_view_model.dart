...
class LoginViewModel extends ChangeNotifier {
      ...
      //try {
        ...
        if (success) {
          Navigator.of(context).pushReplacement(
              CupertinoPageRoute(builder: (_) => TabScreen(userId: auth.userId)));
        }
        else{
          showInSnackBar('${auth.message}',context);
          auth.message="";
        }
      // } catch (e) {
      //   loading = false;
  ...
  forgotPassword(BuildContext context) async {
    ...
    FormState form = formKey.currentState;
    form.save();
    print(Validations.validateEmail(username));
    if (Validations.validateEmail(username) != null) {
